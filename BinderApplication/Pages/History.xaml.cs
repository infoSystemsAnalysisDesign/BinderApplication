...
public partial class History : ContentPage
{
    ...
    private async void UpdateDisplay()
    {
        ...
        var hatedBooks = database.GetCollection<BsonDocument>("Hate");
        ...
        try
        {
            var booksHated = await hatedBooks.Find(filter).ToListAsync();

            if (booksHated.Count > 0)
            {
                foreach (var book in booksHated)
                {
            ...
            else
            {
                ...
                var noEntriesLabel = new Label { Text = "No hated entries found." };
        ...
        catch (Exception ex)
        {
            ...
            var errorLabel = new Label { Text = $"Error retrieving hated entries: {ex.Message}" };
